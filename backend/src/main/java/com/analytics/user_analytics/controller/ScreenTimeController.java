            ...
            if (fromDate != null && toDate != null) {
                ...
                screenTimes = screenTimes.stream()
                        .filter(st -> {
                            boolean inRange = !st.getTimestamp().isBefore(from) && !st.getTimestamp().isAfter(to);
                            if (!inRange) {
                                System.out.println("Filtered out screen time: " + st.getTimestamp() + " (not in range)");
                            }
                            return inRange;
                        })
                        .collect(Collectors.toList());
    ...
    public ResponseEntity<Map<String, Object>> getUserScreenSummary(
        ...
        try {
            ...
            for (Map.Entry<String, Long> entry : categoryTotals.entrySet()) {
                ...
                categoryData.put("averageTimePerVisit", categoryVisits.get(category) > 0 ?
                    formatDuration(duration / categoryVisits.get(category)) : "0");
